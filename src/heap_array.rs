...
pub fn create_cells_array(
    ...
    let mut data = ManuallyDrop::new(vec![initial_value; SIMULATION_HEIGHT * SIMULATION_WIDTH]);
    unsafe {
        Box::from_raw(
            data.as_mut_ptr() as *mut [[Option<Particle>; SIMULATION_HEIGHT]; SIMULATION_WIDTH]
        )
    }
}
...
pub fn create_background_array(
    ...
    unsafe {
        Box::from_raw(data.as_mut_ptr() as *mut [u8; SIMULATION_HEIGHT * SIMULATION_WIDTH * 4])
    }
}
